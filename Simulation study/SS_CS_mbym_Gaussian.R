...
#==== Custom score function INLA_ user defind prior
...
#== score function INLA, default LKJ prior, for comparison ===# 
...
fitME4 <- function(node, parents, group, data) {
  ...
  if (length(parents) == 0) {
    ...
    m <- length(parents) + 1
    if (length(parents) > 2) eta_values <- c(12, 20)
  }
...
data$b0 = -1.03
data$Y2 = data$b0 +  data$u0 + data$e
...
data$Y3 = data$b0 + data$b1*data$Y1 + data$u1*data$Y1 + data$u0 + data$e
...
sigmae = sqrt(0.54) # 0.9
data$e = rnorm(N, 0, sigmae)

data$b0 = -0.32
data$b1 = 0.56

data$Y4 = data$b0 + data$b1*data$Y1 + data$u1*data$Y1 + data$u0 + data$e
...
CPTY6 = list(
  coef = matrix(c(b0, b1, b2), nrow = 3, ncol = ncluster, byrow = TRUE,
                dimnames = list(c("(Intercept)", "Y1","Y5"), as.character(seq(0, ncluster-1)))),
  sd = sde
)
...
CPTY7 = list(
  coef = matrix(c(b0, b1, b2), ncol = ncluster, nrow = 3, byrow = TRUE,
                dimnames = list(c("(Intercept)", "Y5", "Y6"), as.character(seq(0, ncluster-1)))),
  sd = sde
)
data <- data[, !(names(data) %in% c("u0","u1","u2", "e", "b0","b0i", "b1i", "b1", "b2i", "b2"))]
...
CPTY9 = list(
  coef = matrix(c(b0, b1, b2, b3), ncol = ncluster, nrow = 4, byrow = TRUE,
                dimnames = list(c("(Intercept)", "Y1", "Y4","Y8"), as.character(seq(0, ncluster-1)))),
  sd = sde
)
...
CPTY10 = list(
  coef = matrix(c(b0, b1, b2), ncol = ncluster, nrow = 3, byrow = TRUE,
                dimnames = list(c("(Intercept)", "Y4", "Y6"), as.character(seq(0, ncluster-1)))),
  sd = sde
)
...
stats_1<-data.frame(shd_lme, shd_inla, shd_inla1, shd_inla2, shd_inla3,
                    KL_lme, KL_inla, KL_inla1, KL_inla2, KL_inla3, F1_lme, F1_inla,
                    F1_inla1, F1_inla2, F1_inla3, F1_Slme, F1_Sinla, F1_Sinla1, 
                    F1_Sinla2, F1_Sinla3, F1_Slme_1, F1_Sinla_1, F1_Sinla1_1, 
                    F1_Sinla2_1, F1_Sinla3_1)


setwd("path")
out.file2 <- sprintf(
  "Gaussian_MbyM_%s_%s.rds",
