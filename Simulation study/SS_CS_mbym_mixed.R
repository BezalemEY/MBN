...
##=== Custom score function INLA_ default Wishart prior ===#
...
fitME2 <- function(node, parents, group, data) {
  ...
  eta_values <- c(10, 20) # we used 20 for debuging purpose
...
for(k in 1:nsims) {
  ...
  data <- data[, !(names(data) %in% c("u0","u1","u2","u3", "e", "b0", "b1", "b2i", "b2","b3"))]
  ...
  data <- data[, !(names(data) %in% c("u0","u1","u2", "e", "b0", "b1","b2", "ID"))]
...
stats_4<-data.frame(shd_lme, shd_inla, shd_inla1)
